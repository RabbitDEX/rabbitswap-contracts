 ...
 */
abstract contract VRC25Upgradeable is Ownable2StepUpgradeable, IVRC25, ERC165 {
   ...
   */
  function __VRC25_init(string memory name_, string memory symbol_, uint8 decimals_) internal onlyInitializing {
		_name = name_;
		_symbol = symbol_;
		_decimals = decimals_;
  }
	...
	function _chargeFeeFrom(address sender, address recipient, uint256 amount) internal {
		...
		if (amount > 0) {
			...
			emit Fee(sender, recipient, owner(), amount);
		}
	}
	/**
