...
describe("RabbitStaker - Withdraw Functionality", function () {
  ...
  describe("Basic Withdraw Operations", function () {
    ...
    it("should allow withdrawal with minimum vesting period", async function () {
      ...
      // Perform withdrawal
      const tx = await rabbitStaker.connect(user1).withdraw(withdrawAmount, vestingDays);
    ...
    it("should handle multiple withdrawals by same user", async function () {
      ...
      // First withdrawal
      await rabbitStaker.connect(user1).withdraw(firstWithdraw, vestingDays);
      
      // Second withdrawal
      await rabbitStaker.connect(user1).withdraw(secondWithdraw, vestingDays);
    ...
    it("should handle withdrawals from multiple users", async function () {
      ...
      // Both users withdraw
      await rabbitStaker.connect(user1).withdraw(withdrawAmount, vestingDays);
      await rabbitStaker.connect(user2).withdraw(withdrawAmount, vestingDays);
  ...
  describe("Error Cases", function () {
    ...
    it("should revert when insufficient RABBIT in pool", async function () {
      ...
      await rabbitStaker.connect(user1).withdraw(ethers.parseEther("1000"), 180);
      await rabbitStaker.connect(user2).withdraw(ethers.parseEther("4000"), 180); 
      // user2 still has 1000 sRABBIT remaining
      
      // Try to withdraw a smaller amount that user2 has but would exceed available pool
      await expect(rabbitStaker.connect(user2).withdraw(ethers.parseEther("500"), 180))
    ...
    it("should revert when withdrawal results in zero RABBIT amount", async function () {
      ...
      if (expectedOutput === 0n && tinyAmount > 0n) {
        await expect(rabbitStaker.connect(user1).withdraw(tinyAmount, 15))
  ...
  describe("Exchange Rate Impact", function () {
    ...
    it("should handle withdrawal with improved exchange rate from rewards", async function () {
      const { rabbitStaker, rabbitToken, owner, user1, ownerAddress, user1Address } = fixture;
  ...
  describe("Event Emission", function () {
    ...
    it("should emit RabbitPerShareUpdated event on withdrawal", async function () {
      ...
      await expect(rabbitStaker.connect(user1).withdraw(withdrawAmount, vestingDays))
  ...
  describe("Edge Cases", function () {
    ...
    it("should handle withdrawal of entire sRABBIT balance", async function () {
      ...
      await rabbitStaker.connect(user1).withdraw(entireBalance, vestingDays);
    ...
    it("should handle withdrawal exactly at vesting boundaries", async function () {
      ...
      await expect(rabbitStaker.connect(user1).withdraw(withdrawAmount, 15))
        .to.not.be.reverted;
      
      // Test maximum boundary  
      await expect(rabbitStaker.connect(user1).withdraw(withdrawAmount, 180))
        .to.not.be.reverted;
    });
  });
});
