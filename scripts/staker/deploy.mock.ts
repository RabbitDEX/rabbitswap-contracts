...
export const DEPLOY_RABBIT_STAKER = async () => {
  ...
  writeMetadata("MockSRabbitToken", sRabbitTokenAddress);
  writeMetadata("MockRabbitStaker_Proxy", proxyAddress);
  writeMetadata("MockRabbitStaker_Implementation", implementationAddress);
  writeMetadata("MockRabbitStaker_ProxyAdmin", adminAddress);
...
async function main() {
  ...
  console.log("MockSRabbitToken:", await sRabbitToken.getAddress());
  console.log("MockRabbitStaker Proxy:", await rabbitStaker.getAddress());
  console.log(
    "MockRabbitStaker Implementation:",
    await upgrades.erc1967.getImplementationAddress(
      await rabbitStaker.getAddress()
    )
  );
  console.log(
    "MockRabbitStaker ProxyAdmin:",
    await upgrades.erc1967.getAdminAddress(await rabbitStaker.getAddress())
  );

  console.log("\nDeployment completed successfully!");
  console.log("To verify contracts, run: npx hardhat run scripts/staker/verify.mock.ts --network vic-mainnet");
}
